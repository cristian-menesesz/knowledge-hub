...
When creating a new package in the monorepo, follow this process to ensure `package-lock.json` stays
in sync:
...
Edit the generated `package.json` to use the workspace naming convention:

```json
{
  "name": "@knowledge-hub/new-package",
  "version": "0.1.0",
  "private": true,
  "main": "./src/index.ts"
}
...
#### 3. Add to root workspaces (if needed)

If not using glob patterns, add to root `package.json`:
...
```json
{
  "workspaces": [
    "apps/*",
    "packages/*",
    "packages/new-package" // Add explicitly if needed
  ]
}
```

#### 4. **CRITICAL**: Update package-lock.json

```bash
cd ../..  # Return to repository root
npm install  # This syncs package-lock.json with the new workspace
```

⚠️ **This step is mandatory!** Skipping it will cause CI failures with:

```
npm error Missing: @knowledge-hub/new-package@0.1.0 from lock file
```

#### 5. Commit everything together

```bash
git add packages/new-package package-lock.json
git commit -m "feat(new-package): Add new workspace package"
```

**Why this matters:**

- Local development uses `npm install` which auto-syncs lockfile
- CI uses `npm ci` which requires exact lockfile synchronization
- Forgetting `npm install` will cause all CI workflows to fail
